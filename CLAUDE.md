...
All files in this project use **kebab-case** naming:

- **Source files**: `{module-name}.ts`
  - Examples: `css-parser.ts`, `theme-builder.ts`, `report-writer.ts`
- **Test files**: `{module-name}.test.ts` (with `.test` suffix)
  - Examples: `conflicts.test.ts`, `extractor.test.ts`, `edge-cases.test.ts`
- **Fixture files**: `{fixture-name}.css`
  - Examples: `base-theme.css`, `dark-mode.css`, `custom-themes.css`

**Rationale**: kebab-case avoids cross-platform filesystem issues (macOS/Windows are case-insensitive) and is the standard convention for enterprise TypeScript projects.
