...
function copyTemplate(filename: string, force = false): boolean {
...
async function main(): Promise<void> {
  ...
  const files = await p.multiselect({
    ...
    options: [
      ...
      { value: 'claude', label: 'Claude AI Instructions (CLAUDE.md)' },
    ],
    initialValues: ['eslint', 'prettier', 'prettierignore', 'claude'],
    required: true,
  });
  ...
  const updateVersion = await p.confirm({
    message: 'Set version to 0.1.0 in package.json?',
    initialValue: true,
  });

  if (p.isCancel(updateVersion)) {
    p.cancel('Operation cancelled');
    process.exit(0);
  }

  const updateExports = await p.confirm({
    message: 'Add exports field to package.json?',
    initialValue: false,
  });

  if (p.isCancel(updateExports)) {
    p.cancel('Operation cancelled');
    process.exit(0);
  }
  ...
  if (files.includes('claude')) {
    copyTemplate('CLAUDE.md', true);
  }

  s.stop('Configuration files created');

  if (updateScripts || updateEngines || updateVersion || updateExports) {
